...
export default class bestolevel extends Command {
    ...
    async Execute(interaction: ChatInputCommandInteraction) {
        ...
        const data: RunData[] | undefined = runFunction(blackEggs, 0, 0, 0, 0, 'normal');
        if (!data) {
            return await interaction.reply({ content: "Black eggs cannot be less than 1e100" });
        }

        const level         = this.formatNumber(Number(data[0].level));
        const idleLevel     = this.formatNumber(Number(data[0].idleLevel));
        const idleComboLevel = this.formatNumber(Number(data[0].idleComboLevel));
